...
function edit_kontak_perangkat(id) {
  window.location = _base_url + 'admin/c_kontak_perangkat/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'admin/c_kontak_perangkat/add';
    }	
	...
	if (com=='Delete Selected Items')
        {
           ...
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("admin/c_kontak_perangkat/delete/");?>",
					   data: "items="+itemlist,
